...
class TestPerson(unittest.TestCase):
    ...
    def test_get_all_person(self):
        ...
        self.assertDictEqual(person_list[0], {'email': 'alice@bob.com', 'id': '1', 'name': 'Alice', 'notificationType': 'email', 'notifications': 'True', 'password': 'Bob'})
        self.assertDictEqual(person_list[1], {'email': 'aweesome@bob.com', 'id': '2', 'name': 'Bob', 'notificationType': 'email', 'notifications': 'True', 'password': 'Alice'})
    ...
    def test_get_person_with_valid_id(self):
        ...
        self.assertDictEqual(person, {'email': 'alice@bob.com', 'id': '1', 'name': 'Alice', 'notificationType': 'email', 'notifications': 'True', 'password': 'Bob'})
    ...
    def test_put_person_without_id(self):
        ...
        # send the request and check the response status code
        response = self.app.put("/person", data={"name": "Amy", "password": "Bob", "email": "alice@bob.com", "notifications": 'True', "notificationType": "email"})
        self.assertEqual(response.status_code, 200)
    ...
    def test_put_person_with_new_id(self):
        # send the request and check the response status code
        response = self.app.put("/person", data={"id": 3, "name": "Amy", "password": "Bob", "email": "alice@bob.com", "notifications": 'True', "notificationType": "email"})
        self.assertEqual(response.status_code, 200)
